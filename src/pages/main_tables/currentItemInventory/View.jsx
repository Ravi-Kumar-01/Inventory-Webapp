...
import {
	...
	BaseUrl,
	CurrentItemInventoryPaginatedUrl,
...
const CurrentItemInventory = () => {
	...
	const onSearch = async (value) => {
		if (value) {
			...
			const SearchUrl =
				BaseUrl +
				`api/CurrentItemInventory/?timestamp=${timestamp}&search=${value}`;
	...
	const columns = [
		...
		// },
		{
			...
			render: (_, record, index) => {
				let res = companies?.filter(
					(item) => item.id === record.company_supplier_id
				);
				return res.length > 0 ? res[0].name : "";
			},
		},
		{
			...
			render: (_, record, index) => {
				let res = companies?.filter(
					(item) => item.id === record.company_customer_id
				);
				return res.length > 0 ? res[0].name : "";
			},
		},
		{
			...
			render: (_, record, index) => {
				let res = persons?.filter(
					(item) => item.id === record.person_supplier_id
				);
				return res.length > 0 ? res[0].name : "";
			},
		},
		{
			...
			render: (_, record, index) => {
				let res = persons?.filter(
					(item) => item.id === record.person_customer_id
				);
				return res.length > 0 ? res[0].name : "";
			},
		},
		...
		},
		{
			...
			render: (_, record, index) => {
				let res = UOMs?.filter(
					(item) => item.id === record.eaches_received_uom_id
				);
				return res.length > 0 ? res[0].name : "";
			},
		},
		...
		},
		{
			...
			render: (_, record, index) => {
				let res = handlingUnits?.filter(
					(item) => item.id === record.top_handling_unit_id
				);
				return res.length > 0 ? res[0].name : "";
			},
		},
		...
		},
		{
			...
			render: (_, record, index) => {
				let res = handlingUnits?.filter(
					(item) => item.id === record.bottom_handling_unit_id
				);
				return res.length > 0 ? res[0].name : "";
			},
		},
	...
	const processDataForExcel = () => {
		return data.map((record) => {
			...
			const selCostCenter = costCenters?.find(
				(cc) => cc.id === record.cost_center_id
			);
			...
			const selCompanySupplier = companies?.find(
				(item) => item.id === record.company_supplier_id
			);
			selCompanySupplier &&
				(updatedRecord.company_supplier_id = selCompanySupplier.name);

			let selCompanyCustomer = companies?.find(
				(item) => item.id === record.company_customer_id
			);
			selCompanyCustomer &&
				(updatedRecord.company_customer_id = selCompanyCustomer.name);

			const selPersonSupplier = persons?.find(
				(item) => item.id === record.person_supplier_id
			);
			selPersonSupplier &&
				(updatedRecord.person_supplier_id = selPersonSupplier.name);

			let selPersonCustomer = persons?.find(
				(item) => item.id === record.person_customer_id
			);
			selPersonCustomer &&
				(updatedRecord.person_customer_id = selPersonCustomer.name);

			let regularUOM = UOMs?.find((item) => item.id === record.regular_uom_id);
			regularUOM && (updatedRecord.regular_uom_id = regularUOM.name);

			let eachesReceivedUOM = UOMs?.find(
				(item) => item.id === record.eaches_received_uom_id
			);
			eachesReceivedUOM &&
				(updatedRecord.eaches_received_uom_id = eachesReceivedUOM.name);

			let eachesRemainingUOM = UOMs?.find(
				(item) => item.id === record.eaches_remaining_uom_id
			);
			eachesRemainingUOM &&
				(updatedRecord.eaches_remaining_uom_id = eachesRemainingUOM.name);

			let selTopHandlingUnit = handlingUnits?.find(
				(item) => item.id === record.top_handling_unit_id
			);
			selTopHandlingUnit &&
				(updatedRecord.top_handling_unit_id = selTopHandlingUnit.name);

			let selBottomHandlingUnit = handlingUnits?.find(
				(item) => item.id === record.bottom_handling_unit_id
			);
			selBottomHandlingUnit &&
				(updatedRecord.bottom_handling_unit_id = selBottomHandlingUnit.name);
			...
			let selWarehouse = warehouses?.find(
			...
			let selPosition = positions?.find(

...
import { InboundRegistrationUrl, BaseUrl } from "../../utils/network";
...
const InboundRegistration = () => {
  ...
  let tableName = InboundRegistrationUrl.split("/")
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/InboundRegistration/` +
        `?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      setData(res);
    } else {
      let res = await CRUD("get", InboundRegistrationUrl, null, null);
      setData(res);
    }
  };
  //Sort Operation
  ...
  const handleOk = () => {
    form.validateFields().then(async (values) => {
      ...
      let changedValues = {
        ...values,
        datetime_in: values.datetime_in.format("YYYY-MM-DDThh:mm"),
      };
      if (editingItem) {
        setEditingItem(null);
        let result = await CRUD(
          "put",
          InboundRegistrationUrl,
          changedValues,
          editId
        );
        if (result) {
  ...
  const handleEdit = (record) => {
    ...
    let changedValues = {
      ...record,
      datetime_in: moment(record.datetime_in),
    };
  ...
  const getItem = useCallback(async () => {
    ...
    const response = await CRUD("get", InboundRegistrationUrl, null, null);

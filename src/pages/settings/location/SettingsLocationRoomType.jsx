...
const { Search } = Input;
const { Option } = Select;
const SettingsLocationRoomType = () => {
  ...
  let tableName = SettingsLocationRoomTypeUrl.split("/")
    .filter((part) => part.length > 0)
    .pop();

  let roomTypes = useSelector((state) => state.settings.roomTypes);
  ...
  const onSearch = async (value) => {
    if (value) {
      ...
      const SearchUrl =
        BaseUrl +
        `api/SettingsLocationRoomType/?timestamp=${timestamp}&search=${value}`;
      let res = await search("get", SearchUrl, null, null);
      dispatch(getRoomTypes(res));
    } else {
      let res = await CRUD("get", SettingsLocationRoomTypeUrl, null, null);
      dispatch(getRoomTypes(res));
    }
  };
  ...
  const getItem = useCallback(async () => {
    ...
    roomTypes && roomTypes.length > 0 ? setData(roomTypes) : setData([]);

    setLoading(false);
  }, [roomTypes, isLoading, loading]);
  ...
  return (
    <div>
      <PageLayout
				title="SettingsLocationRegionType"
				titleRightAction={() =>
					userPerm &&
...
export default SettingsLocationRoomType;
